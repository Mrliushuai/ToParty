...
+ (void)addLoadMoreForScrollView:(UIScrollView *)scrollView
    ...
    DDRefreshFooter *footer = [DDRefreshFooter footerWithRefreshingBlock:^{
        if (loadMoreCallBackBlock) {
            loadMoreCallBackBlock();
        }
    }];
    [footer setTitle:@"" forState:MJRefreshStateIdle];
    [footer setTitle:@"桐聚正在为您加载数据" forState:MJRefreshStateRefreshing];
...
+ (BOOL)isCurrentUserWithUserId:(NSInteger)userId {
    if ([DDUserInfoManager sharedManager].isLogin == NO) {
        return NO;
    }
    if (userId == 0) {
        return NO;
    }
    return [DDUserInfoManager sharedManager].currentUserInfo.user_id == userId;
}
