...
NSString * const DDMenuWillAppearNotification = @"DDMenuWillAppearNotification";
NSString * const DDMenuDidAppearNotification = @"DDMenuDidAppearNotification";
NSString * const DDMenuWillDisappearNotification = @"DDMenuWillDisappearNotification";
NSString * const DDMenuDidDisappearNotification = @"DDMenuDidDisappearNotification";
...
/// 背景色效果
static DDMenuBackgrounColorEffect   gBackgroundColorEffect = DDMenuBackgrounColorEffectSolid;
/// 是否显示阴影
...
typedef enum {
    ...
    DDMenuViewArrowDirectionNone,
    DDMenuViewArrowDirectionUp,
    DDMenuViewArrowDirectionDown,
    DDMenuViewArrowDirectionLeft,
    DDMenuViewArrowDirectionRight,
    
} DDMenuViewArrowDirection;
...
@interface FDDMenuOverlay : UIView
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[DDMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
@implementation DDMenuView {
    DDMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    DDMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([FDDMenu hasShadow]) {
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [FDDMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    const CGFloat widthPlusArrow = contentSize.width + [FDDMenu arrowSize];
    const CGFloat heightPlusArrow = contentSize.height + [FDDMenu arrowSize];
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = DDMenuViewArrowDirectionUp;
        ...
        _contentView.frame = (CGRect){0, [FDDMenu arrowSize], contentSize};
        
        self.frame = (CGRect) {
            
            point,
            contentSize.width,
            contentSize.height + [FDDMenu arrowSize]
        };
        
    } else if (heightPlusArrow < rectY0) {
        
        _arrowDirection = DDMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = DDMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _contentView.frame = (CGRect){[FDDMenu arrowSize], 0, contentSize};
        
        self.frame = (CGRect) {
            
            point,
            contentSize.width + [FDDMenu arrowSize],
            contentSize.height
        };
        
    } else if (widthPlusArrow < rectX0) {
        
        _arrowDirection = DDMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = DDMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(DDMenuSelectedItem)selectedItem {
    ...
    FDDMenuOverlay *overlay = [[FDDMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:DDMenuWillAppearNotification object:nil];
    ...
    [UIView animateWithDuration:0.2 animations:^(void) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DDMenuDidAppearNotification object:nil];
    }];
}

- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        [[NSNotificationCenter defaultCenter] postNotificationName:DDMenuWillDisappearNotification object:nil];
        
        __weak typeof(self) weakSelf = self;
        void (^removeView)(void) = ^(void) {
            if ([weakSelf.superview isKindOfClass:[FDDMenuOverlay class]])
                [weakSelf.superview removeFromSuperview];
            [weakSelf removeFromSuperview];
            
            [[NSNotificationCenter defaultCenter] postNotificationName:DDMenuDidDisappearNotification object:nil];
        };
    ...
    [FDDMenu sharedMenu].isShow = NO;
}

- (void)performAction:(id)sender {
    [self dismissMenu:YES];
    UIButton *button = (UIButton *)sender;
    FDDMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *)mkContentView {
    ...
    UIFont *titleFont = [FDDMenu titleFont];
    
    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    
    for (FDDMenuItem *menuItem in _menuItems) {
    ...
    for (FDDMenuItem *menuItem in _menuItems) {
        ...
        const CGFloat itemHeight = MAX(titleSize.height, imageSize.height) + [FDDMenu menuItemMarginY] * 2;
    ...
    UIImage *selectedImage = [DDMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight}];
    
    UIImage *gradientLine = [DDMenuView gradientLineWithColor:[FDDMenu separatorColor]];
    ...
    for (FDDMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.title.length) {
            ...
            if (!menuItem.enabled && !menuItem.image) {
                ...
                titleFrame = (CGRect){
                    kMarginX * 2,
                    [FDDMenu menuItemMarginY],
                    maxItemWidth - kMarginX * 4,
                    maxItemHeight - [FDDMenu menuItemMarginY] * 2
                };
        ...
        if (menuItem.image) {
            ...
            const CGRect imageFrame = {kMarginX * 2, [FDDMenu menuItemMarginY], maxImageWidth, maxItemHeight - [FDDMenu menuItemMarginY] * 2};
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == DDMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == DDMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == DDMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == DDMenuViewArrowDirectionRight) {
...
+ (UIImage *)selectedImage:(CGSize)size {
    ...
    CGContextSetFillColorWithColor(context, [[FDDMenu selectedColor] CGColor]);
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [FDDMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([FDDMenu backgrounColorEffect] == DDMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == DDMenuViewArrowDirectionUp) {
        ...
        const CGFloat arrowX0 = arrowXM - [FDDMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [FDDMenu arrowSize];
        const CGFloat arrowY0 = Y0;
        const CGFloat arrowY1 = Y0 + [FDDMenu arrowSize] + kEmbedFix;
        ...
        const CGFloat arrowX0 = arrowXM - [FDDMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [FDDMenu arrowSize];
        const CGFloat arrowY0 = Y1 - [FDDMenu arrowSize] - kEmbedFix;
        ...
        Y1 -= [FDDMenu arrowSize];
        
    } else if (_arrowDirection == DDMenuViewArrowDirectionLeft) {
        
        const CGFloat arrowYM = _arrowPosition;
        const CGFloat arrowX0 = X0;
        const CGFloat arrowX1 = X0 + [FDDMenu arrowSize] + kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [FDDMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [FDDMenu arrowSize];
        ...
        X0 += [FDDMenu arrowSize];
        
    } else if (_arrowDirection == DDMenuViewArrowDirectionRight) {
        
        const CGFloat arrowYM = _arrowPosition;
        const CGFloat arrowX0 = X1;
        const CGFloat arrowX1 = X1 - [FDDMenu arrowSize] - kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [FDDMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [FDDMenu arrowSize];
    ...
    UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:bodyFrame
                                                          cornerRadius:[FDDMenu cornerRadius]];
    ...
    if (_arrowDirection == DDMenuViewArrowDirectionLeft ||
        _arrowDirection == DDMenuViewArrowDirectionRight) {
...
#pragma mark - YCXMenu
/// MenuView
static FDDMenu                      *gMenu;

@implementation FDDMenu {
    DDMenuView *_menuView;
    BOOL         _observing;
}
...
+ (void)reset {
    ...
    gBackgroundColorEffect = DDMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(DDMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[DDMenuView alloc] init];
