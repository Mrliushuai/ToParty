...
- (void)showImage{
    ...
    DDDragImageView *dragImageView = [arrImages objectAtIndex:0];
    ...
    for (int i=0; i<arrImages.count; i++) {
        ...
        DDDragImageView *imageview = [arrImages objectAtIndex:i];
...
-(UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    NSLog(@"%@",fitView);
    if ([fitView isKindOfClass:[DDDragImageView class]]) {
        _ImgView = (DDDragImageView *)fitView;
...
- (void)reviseCirclePoint{
    BOOL isClockwise;
    DDDragImageView *imageviewFirst = [arrImages objectAtIndex:0];
...
//平衡动画
- (void)animateWithDuration:(CGFloat)time animateDelay:(CGFloat)delay changeIndex:(NSInteger)change_index toIndex:(NSInteger)to_index circleArray:(NSMutableArray *)array clockwise:(BOOL)is_clockwise{
    DDDragImageView *change_cell = [array objectAtIndex:change_index];
    DDDragImageView *to_cell     = [array objectAtIndex:to_index];
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    for (int i = 0; i < arrImages.count; ++i)
    {
        ...
        DDDragImageView *change_cell = [arrImages objectAtIndex:i];
        
        DDDragImageView *to_cell     = [arrImages objectAtIndex:iDest];
